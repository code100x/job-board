import Image from "next/image";
import Link from "next/link";
import GithubBadge from "./GithubBadge";
import { TextGenerationEffect } from "./ui/TextGenerationEffect";

export const Hero = () => {
  return (
    <>
      <div className="flex flex-wrap justify-between ">
        <div className="flex items-center w-full lg:w-1/2 justify-center">
          <div className="max-w-full mb-8">
            <section className="flex flex-col justify-start items-center gap-8">
              <GithubBadge />
              <div className="flex flex-col justify-start items-center gap-10 mt-10">
                <p className="text-xl sm:text-2xl lg:text-3xl tracking-tight font-semibold text-slate-700">
                  Join the #1 Open Source Job-Platform
                </p>
                <h1 className="text-6xl sm:text-7xl lg:text-8xl text-center tracking-tighter bg-gradient-to-r from-indigo-600 via-violet-500 to-blue-700 bg-clip-text text-transparent font-black">
                  100xJobs
                </h1>
                <TextGenerationEffect
                  words="India's most rapidly growing developer community"
                  className="text-lg sm:text-xl lg:text-2xl text-center text-slate-700/90 font-bold tracking-loose"
                />
              </div>
            </section>

            <div className="flex flex-row items-start mt-28 gap-10 sm:mt-32 sm:gap-9 sm:justify-center justify-center space-y-3 sm:space-x-4 sm:space-y-0 sm:items-center sm:flex-row">
              <Link
                href="/jobs"
                className="px-8 py-4 text-lg font-medium text-center text-white bg-indigo-600 hover:bg-indigo-500 rounded-md "
              >
                Explore{" "}
              </Link>
              <Link
                href="https://github.com/code100x/job-board/"
                target="_blank"
                rel="noopener"
                className="flex items-center space-x-2 text-gray-500 hover:bg-slate-100  dark:text-gray-400"
              >
                <svg
                  role="img"
                  width="24"
                  height="24"
                  className="w-5 h-5"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <title>GitHub</title>
                  <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
                </svg>
                <span> View on Github</span>
              </Link>
            </div>
          </div>
        </div>
        <div className="flex items-center justify-center w-full lg:w-1/2">
          <Image
            src="/job.webp"
            width="700"
            height="700"
            className="ml-10 object-cover"
            alt="Hero Illustration"
            loading="eager"
          />
        </div>
      </div>
    </>
  );
};
